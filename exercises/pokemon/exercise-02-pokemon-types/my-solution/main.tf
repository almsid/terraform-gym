...
locals {
  ...
  team_names = [for p in var.pokemon_team : p.name]

  # TODO: Calculate total HP of the team
  # Hint: Use sum() with a for expression
  team_total_hp = sum([for p in var.pokemon_team : p.hp])

  # TODO: Filter for just fire-type Pokemon
  fire_types = [for p in var.pokemon_team : p.name if p.type == "fire"]

  # TODO: Create a map of name -> HP for quick lookup
  hp_lookup = { for p in var.pokemon_team : p.name => p.hp }
}
